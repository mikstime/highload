...
ученика, месты работы учителя, и так далее. Также опустим сущность видео, поскольку
статистика просмотров и комментариев не важна в рамках проекта.
...
(конкретные СУБД, шардинг, расчет нагрузки, обоснование реализуемости 
на основе результатов нагрузочного тестирования)
...
Для хранения информации о студентах, преподавателях и тестах будем использовать
хранилище типа ключ-значение/колоночное в силу большей производительности, чем 
документоориентированные базы данных. При необходимости изменения данных,
приложение будет считывать старую версию, вносить изменение и перезаписывать
значение.
...
Для экономии, данные старше одного месяца будут храниться на жестких дисках в
отдельном датацентре (сетевая задержка не так важна для данных запросов).
...
14.6 млн пользователей. Из них 0.65 млн – преподаватели, оставшиеся 13.8 мле – ученики
...
Сервер с 1 ведущим узлом (Master-slave) без проблем способен выдержать 
подробную нагрузку.
...
Клиентское приложение (сайт) написан на Javascript, в частности использует 
разработки Facebook: React, Redux, Relay. Благодаря тому, что новые уроки
...
Материалы лекция являются часто запрашиваемыми, поэтому информация будет
храниться на 4-х терабайтных ссд (RAID 5/6).
...
RAID 6 (Емкость 36 ТБ, скорость записи x9, 2 отказа).
...
отказа у 1 дисков из 65. Таким образом нам понадобится 36 таких стендов.
