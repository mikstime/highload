...
В качестве географического DNS сервера используется **Amazon Route 53**,
популярность которого значительно выше остальных решений на рынке.
...
работать с соединением на достаточно низком уровне позволяет писать очень 
производительные приложения. При этом скорость написания кода на Rust
...
Таким образом мы получаем два хранилища, каждое из которых включает в себя 36 стендов.
...
| Тип сервера |      Хранилище      |              Количество               |
| :---------: | :------------------ | :------------------------------------ |
|   Frontend  |  4 ТБ SSD RAID 5/6  |                  10                   |
|   Backend   |   36 ТБ SSD RAID 6  |                   3                   |
|   S3 стенд  | 1260 ТБ HDD RAID 10 | 36 серверов на хранилище, 2 хранилища |
...
[Причины выбрать Route 53](https://cloudacademy.com/blog/route53-dns-migration/)

[Поддержка GeoDNS в Route 53](https://docs.aws.amazon.com/Route53/latest/DeveloperGuide/routing-policy.html#routing-policy-geo)

[Route 53 vs ClouDNS](https://stackshare.io/stackups/amazon-route-53-vs-cloudns)
